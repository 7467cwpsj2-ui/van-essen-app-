...
export function DirectMessagePanel({
  ...
  useRealtimeRefresh("direct_messages", teamMemberId, "team_member_id");
  ...
  const send = async () => {
    ...
    try {
      ...
      if (pending) {
        ...
        const path = `${teamMemberId}/${crypto.randomUUID()}.${ext}`;
  ...
  return (
    <div className="panel">
      ...
      {messages.length === 0 && <div className="empty-hint">Nog geen berichten — begin het gesprek.</div>}
      ...
      {pending && (
        <FilePreview previewUrl={pending.previewUrl} fileType={pending.fileType} fileName={pending.fileName} onClear={() => setPending(null)} />
      )}
      <div className="chat-input-row">
        <FileCaptureButtons accept="image/*,application/pdf" onPicked={handlePicked} busy={busy} variant="icon" />
        <textarea
          ref={textareaRef}
          rows={1}
